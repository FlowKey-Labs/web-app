...
import { AddClient, GroupData } from '../../types/clientTypes';
...
export default function ClientDrawer({
  ...
  const parentDrawer =
    drawerStack.length > 1 ? drawerStack[drawerStack.length - 2] : null;
  ...
  const { data: clientData, isLoading: isClientLoading } = useGetClient(
      ...
      if (name === 'client_ids' && value.client_ids) {
        ...
        if (clientIds.length > 0 && clientsData) {
          ...
          const currentContactPerson =
            groupFormMethods.getValues('contact_person');
    ...
    try {
      ...
      if (isFromSessionDrawer && parentDrawer?.entityId) {
        ...
        const sessionIds = data.sessions
          .filter(
            (session) => session && typeof session === 'object' && session.value
          )
  ...
  const handleSubmitGroup = groupHandleSubmit(async (data) => {
    try {
      ...
      if (!data.contact_person) {
        setFormErrors({
          contact_person: 'Please select a contact person for the group',
        });
        notifications.show({
          title: 'Error',
                    ...
                    <div className='flex items-start'>
                      ...
                      <div className='ml-3'>
                        ...
                        <div className='mt-1 text-sm text-blue-700'>
                          <p>
                            This client will be automatically assigned to the
                            session you're creating.
                          </p>
                        </div>
                      </div>
                    </div>
                ...
                {isFromSessionDrawer && (
                  <div className='bg-blue-50 p-3 mb-4 rounded-md border border-blue-200'>
                    <div className='flex items-start'>
                      ...
                      <div className='ml-3'>
                        <h3 className='text-sm font-medium text-blue-800'>
                          Creating Group for Session
                        </h3>
                        <div className='mt-1 text-sm text-blue-700'>
                          <p>
                            This group will be automatically assigned to the
                            session you're creating.
                          </p>
                        </div>
                      </div>

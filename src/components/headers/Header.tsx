...
const NOTIFICATION_TABS = [
  ...
  },
  {
    ...
    emptyDescription:
      "Cancelled and rescheduled booking notifications will appear here",
  },
  {
    key: "system",
    label: "System",
    ...
    emptyDescription:
      "System updates, expired bookings, and payment confirmations will appear here",
  },
];
...
const Header = ({ showSearch = true }: HeaderProps) => {
  ...
  const unreadCount =
    ...
    notifications.filter((n: ApiNotification) => !n.is_read).length;

  const categorizedNotifications = notifications.reduce(
    (acc: Record<string, ApiNotification[]>, notification: ApiNotification) => {
      const category =
        NOTIFICATION_CATEGORIES[
          notification.type as keyof typeof NOTIFICATION_CATEGORIES
        ] || "system";
  ...
  Object.keys(categorizedNotifications).forEach((category) => {
    categorizedNotifications[category].sort(
  ...
  const handleMarkAsRead = async (notificationId: number) => {
    try {
      queryClient.setQueryData(
        ...
        (
          oldData:
            | { results: ApiNotification[]; unread_count: number }
            | undefined
        ) => {
          if (!oldData) return oldData;

          const updatedResults = oldData.results.map(
          ...
          const newUnreadCount = updatedResults.filter(
            ...
            {notifications.length === 0 ? (
              <div className="h-[400px] flex flex-col items-center justify-center text-gray-500 px-8">
                ...
                <p className="text-sm text-center max-w-xs leading-relaxed">
                  You'll see booking updates and system messages here
                </p>
              </div>
            ) : (
              <Tabs
                      ...
                      <Tabs.Panel key={tab.key} value={tab.key} className="p-0">
                        {!hasNotifications ? (
                          ...
                          <div className="divide-y divide-gray-100">
                            {categorizedNotifications[tab.key]?.map(
                              (notification: ApiNotification) => (
                                <div
                                  key={notification.id}
                                ...
                                >
                                  <div className="flex items-start gap-4">
                                    <div className="flex-1 min-w-0">
                                      ...
                                      <h4
                                        className={`text-sm font-semibold mb-2 cursor-pointer hover:text-secondary ${
                                          !notification.is_read
                                            ? "text-gray-900"
                                            : "text-gray-700"
                                        }`}

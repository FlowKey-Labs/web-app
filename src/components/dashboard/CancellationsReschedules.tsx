...
export function CancellationsReschedules() {
  ...
  // Debug logging
  console.log("CancellationsReschedules Debug:", {
    rawData,
    data,
    responseData,
    hasTopMetrics: !!responseData?.top_metrics,
    topMetricsType: typeof responseData?.top_metrics,
  });
  ...
  // Safe array access with additional debugging
  const safeCancellations = Array.isArray(time_series?.cancellations)
    ? time_series.cancellations
    : [];
  const safeReschedules = Array.isArray(time_series?.reschedules)
    ? time_series.reschedules
    : [];
  const safeCancelledSessions = Array.isArray(top_metrics?.cancelled_sessions)
    ? top_metrics.cancelled_sessions
    : [];
  const safeRescheduledSessions = Array.isArray(
    top_metrics?.rescheduled_sessions
  )
    ? top_metrics.rescheduled_sessions
    : [];

  console.log("Safe arrays:", {
    safeCancellations: safeCancellations.length,
    safeReschedules: safeReschedules.length,
    safeCancelledSessions: safeCancelledSessions.length,
    safeRescheduledSessions: safeRescheduledSessions.length,
  });

  // Create a map to combine data by date
  const dateMap = new Map();

  safeCancellations.forEach((item) => {
          ...
          <Grid gutter="md">
            <Grid.Col span={{ base: 12, md: 6 }}>
              <Card withBorder radius="md" p="md">
                ...
                <Stack gap="sm">
                  {safeCancelledSessions.length > 0 ? (
                    safeCancelledSessions.map((session) => (
                      <div
            ...
            <Grid.Col span={{ base: 12, md: 6 }}>
              <Card withBorder radius="md" p="md">
                ...
                <Stack gap="sm">
                  {safeRescheduledSessions.length > 0 ? (
                    safeRescheduledSessions.map((session) => (
                      <div
                        key={session.session__id}
                        className="flex justify-between items-center p-2 hover:bg-gray-50 rounded"
                      >
